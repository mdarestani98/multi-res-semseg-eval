...
class STDCSegmentationBase(nn.Module):
    """
...
class CustomSTDCSegmentation(STDCSegmentationBase):
    ...
    def __init__(self, backbone: STDCBackbone, num_classes: int, context_fuse_channels: int = 128,
                 ffm_channels: int = 256, aux_head_channels: int = 64, detail_head_channels: int = 64,
                 use_aux_heads: bool = True, dropout: float = 0.2):
        super().__init__(backbone=backbone,
                         num_classes=num_classes,
                         context_fuse_channels=context_fuse_channels,
                         ffm_channels=ffm_channels,
                         aux_head_channels=aux_head_channels,
                         detail_head_channels=detail_head_channels,
...
class STDC1Seg(CustomSTDCSegmentation):
    ...
    def __init__(self, num_classes: int, in_channels: int = 3, dropout: float = 0.2, aux: bool = True,
                 pretrained: bool = False):
        backbone = STDC1Backbone(in_channels=in_channels, out_down_ratios=[8, 16, 32], pretrained=pretrained)
        super().__init__(backbone, num_classes, use_aux_heads=aux, dropout=dropout)
...
class STDC2Seg(CustomSTDCSegmentation):
    ...
    def __init__(self, num_classes: int, in_channels: int = 3, dropout: float = 0.2, aux: bool = True,
                 pretrained: bool = False):
        backbone = STDC2Backbone(in_channels=in_channels, out_down_ratios=[8, 16, 32], pretrained=pretrained)
        super().__init__(backbone, num_classes, use_aux_heads=aux, dropout=dropout)
