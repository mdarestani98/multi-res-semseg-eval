...
class DualResNet23(nn.Module):
    ...
    def forward(self, x):
        ...
        x_ = F.interpolate(x_, final_size, mode='bilinear', align_corners=True)

        if self.augment:
            x_extra = self.seghead_extra(temp)
            x_extra = F.interpolate(x_extra, final_size, mode='bilinear', align_corners=True)
            return x_, x_extra
        return x_
...
def check():
    model = DualResNet39(BasicBlock, [3, 4, 6, 3], num_classes=19, planes=64, spp_planes=128,
                         head_planes=256, augment=False).cuda()
    model.train()
